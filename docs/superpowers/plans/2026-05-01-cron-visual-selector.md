# Cron 可视化选择器 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 将 ScheduleView.vue 中的原始 Cron 文本输入替换为三 Tab 可视化选择器组件，让普通用户无需了解 Cron 语法即可配置录制计划。

**Architecture:** 新建 `CronSelector.vue` 组件封装全部交互逻辑（预设卡片、自定义时间构建器、高级输入框、实时预览条），通过 `v-model` 对外暴露 Cron 字符串。`ScheduleView.vue` 仅做最小改动：引入组件并替换原有的 `el-input` 字段。

**Tech Stack:** Vue 3 (script setup), Element Plus (el-time-picker, el-input), CSS scoped variables (--color-primary, --color-border 等已有 CSS 变量)
...
| 文件 | 操作 | 职责 |
|------|------|------|
| `src/components/CronSelector.vue` | 新建 | 三 Tab 选择器，v-model 绑定 Cron 字符串 |
| `src/views/ScheduleView.vue` | 修改（第 119-122 行） | 引入并使用 CronSelector，移除 cron-hint |

---

## Task 1：新建 CronSelector.vue 组件

**Files:**
- Create: `src/components/CronSelector.vue`

- [ ] **Step 1：创建组件文件，写入完整实现**

创建 `src/components/CronSelector.vue`，内容如下：
...
- [ ] **Step 2：启动开发服务器，在浏览器验证组件能被正常导入（无控制台报错）**

```bash
npm run dev
```

打开浏览器开发者工具 Console，确认无报错。暂不需要在页面看到效果，Task 2 会集成。

- [ ] **Step 3：Commit**

```bash
git add src/components/CronSelector.vue
git commit -m "feat: add CronSelector component with preset/custom/advanced tabs"
```

---

## Task 2：修改 ScheduleView.vue 集成 CronSelector

**Files:**
- Modify: `src/views/ScheduleView.vue`

- [ ] **Step 1：在 script 顶部引入 CronSelector**

在 `src/views/ScheduleView.vue` 第 7 行（`import { Plus } from ...` 之后）插入：

```js
import CronSelector from '@/components/CronSelector.vue'
```

- [ ] **Step 2：替换表单中的 Cron 字段**

找到以下代码块（约第 119-122 行）：
...
- [ ] **Step 3：删除 cron-hint CSS（已无引用）**

在 `<style scoped>` 中找到并删除：
...
打开 `/schedules` 页面，点击「新建计划」，验证以下场景：

1. **快速选择 Tab**：点击任意预设卡片 → 底部预览条显示正确的中文描述和 Cron 值，卡片高亮
2. **自定义时间 Tab**：修改时间为 `08:30`，取消勾选周六/周日 → 预览条显示 `工作日 08:30 触发`，Cron 值为 `30 8 * * 1,2,3,4,5`
3. **高级 Tab**：输入 `*/15 * * * *` → 预览条显示 `每 15 分钟触发`
4. **高级 Tab**：输入无效内容 `abc` → 预览条变红色显示错误状态
5. **编辑已有计划**：点击表格中某行「编辑」→ 表单 Cron 字段正确回显（显示在高级 Tab 的输入框中）
6. **保存**：填写摄像头、名称、分段时长后点击「创建」→ 计划出现在列表中，cron_expr 值正确

- [ ] **Step 5：Commit**

```bash
git add src/views/ScheduleView.vue
git commit -m "feat: integrate CronSelector into ScheduleView, replace raw cron input"
```
...
- **Spec 覆盖**：快速选择 6 预设 ✓、自定义时间+星期 ✓、高级 Cron 输入 ✓、实时预览条（有效/无效状态）✓、v-model 接口 ✓、Tab 切换不丢状态 ✓
- **占位符**：无 TBD / TODO
- **类型一致性**：`customDays` 全程 `number[]`，`buildCustomCron` 返回 string，`emit('update:modelValue', string)` 与 `modelValue: String` prop 一致
